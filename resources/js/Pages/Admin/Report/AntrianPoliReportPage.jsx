...
export default function AntrianPoliReportPage() {
    ...
    useEffect(() => {
        axios.get(route("admin.report.antrian-poli.getPolis"))
            .then(response => {
                setPolis([{id: "all", nama_poli: "Semua Poli"}, ...response.data]);
            });
    }, []);

    useEffect(() => {
        if (poliId && poliId !== "all") {
            axios.get(route("admin.report.antrian-poli.getDoktersByPoli", { poli_id: poliId }))
                .then(response => {
                    setDokters([{id: "all", name: "Semua Dokter"}, ...response.data]);
                    setDokterId("all");
                });
        } else {
            setDokters([]);
    ...
    const generateReport = () => {
        ...
        axios.post(route("admin.report.antrian-poli.generate"), payload)
            .then(response => {
        ...
        <div className="p-4 max-w-4xl mx-auto">
                    ...
                    <>
                        <div className="mb-4 max-w-sm">
                            <SelectOptions
                                label="Bulan Mulai"
                                value={monthFrom}
                                onChange={e => setMonthFrom(parseInt(e.target.value))}
                            >
                                {[
            ...
            {reportData.length > 0 && (
                <div className="bg-white p-6 rounded shadow overflow-auto">
                    <h2 className="text-lg font-semibold mb-2">Hasil Laporan</h2>
                    <p>Total Data: {total}</p>
                    <table className="w-full border-collapse border border-gray-300 mt-2">
                        <thead>
                            <tr>
                                <th className="border border-gray-300 p-2">ID</th>
                                <th className="border border-gray-300 p-2">Kode Antrian</th>
                                <th className="border border-gray-300 p-2">Nama Pasien</th>
                                <th className="border border-gray-300 p-2">Tanggal Kunjungan</th>
                        ...
                        <tbody>
                            {reportData.map(item => (
                                <tr key={item.id}>
                                    ...
                                    <td className="border border-gray-300 p-2">{item.kd_antrian}</td>
                                    <td className="border border-gray-300 p-2">{item.nama_pasien}</td>
                                    <td className="border border-gray-300 p-2">{item.tanggal_kunjungan}</td>
